...
class Dataset:
    ...
    def create_dataset_output_folder(self):
        #TODO
        return

    def split_dataset_between_train_test_valid(self):
        #TODO: this go at the end after we quit the program.
        print('Exporting dataset...')
        return

    def add_frame_to_dataset(self):
        #TODO
        return
...
def main():
    ...
    while True:
        ...
        for i in range(0, len(bounding_box_set_traffic_signs)):   
            ...
            sign_orientation = int(orientation_taken_by_actors_list) # int(bb.orientation.yaw)
            sign_orientation = int(bb.rotation.yaw)
            tgt_tuple_list.append( (speed_limit_str, int(bb.location.x), int(bb.location.y), sign_orientation) )
